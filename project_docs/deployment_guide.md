...
ecom:
  txpg:
    ...
    # границы периода и время каждой операции в выписке на разницу поясов.
    zone: ${ECOM_TXPG_ZONE:Asia/Baku}
...
| `ECOM_TXPG_ZONE` | не читается | не читается | не читается | необязательна | `Asia/Baku` — часовой пояс, в котором база шлюза хранит даты |
