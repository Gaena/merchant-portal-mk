...
- **PostgreSQL** — одна база данных на все сервисы.
...
- Сервис `ecom` читает базу платёжного шлюза TXPG — только чтение — для мерчантов с собственным
  онлайн-эквайрингом, чьи платежи портал не создаёт, и отдаёт их выписку по API: одна строка на
  заказ, операции заказа — отдельным запросом.
- Выписка берёт только завершённые заказы в финальном статусе; неоплаченные и пробные заказы в неё
  не попадают.
...
- **Состояние на 13.09.2026:** запрос выписки предварительный — ожидается окончательный SQL и
  сопоставление полей от провайдера; словарь статусов провайдера не утверждён, поэтому статус
  заказа определяется по суммам одобренных операций. Экран вкладки «E-commerce» к API `ecom` ещё не
  подключён. Выбор терминала из справочника провайдера при заведении терминала поддержан в API,
  форма на экране появится после согласования справочника.
