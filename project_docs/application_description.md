...
| `terminals` | `directory` | `pbl` — логин, пароль и статус для заказов у шлюза; `ecom` — `merchant_rid` для скоупа выписки |
...
├── types/                   ← DTO, роли, транзакции
└── utils/                   ← разбор ответов (mapTransaction, terminals), исходы денежных операций, экспорт в Excel
```
