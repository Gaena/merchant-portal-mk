 ...
 * принадлежат ему. Администратор выбирает строку из слепка (`providerRid`) и вводит только
 * пароль, а название с логином подставляются оттуда. Поэтому они и не `@NotBlank`: с `providerRid`
...
public record CreateTerminalRequest(
        ...
        /** Игнорируется, когда передан `providerRid`: название приходит от провайдера. */
        String name,

        /** Игнорируется, когда передан `providerRid`: логин приходит от провайдера. */
        String login,
