...
- `/api/v1/payment-links/redirect` (страница возврата плательщика, рендерится на сервере).
- Любые статические ресурсы.

⚠ `GET /api/v1/transactions/{identifier}/status` **больше не публичный** (P0-2, 15.08.2026):
он отдаёт PII, ходит в TXPG и пишет в БД, поэтому закрыт под JWT + проверку компании.
Страница возврата статус по HTTP не запрашивает — он подставляется при серверном рендере.
...
| **AUDITOR** | ✅ Разрешено (все компании) | ❌ Запрещено | ❌ Запрещено | ❌ Запрещено |

> Просмотр у `SYSTEM_ADMIN` и `AUDITOR` — по всем компаниям, у остальных ролей — в пределах терминалов своей компании.
> Исключение (баг P0-4): `GET /api/v1/payment-links/{id}/transactions` проверяет несуществующие роли `MERCHANT_ADMIN`/`MERCHANT_USER` и отдаёт 403 всем, кроме `SYSTEM_ADMIN`.

#### Правила валидации:
1. **Проверка принадлежности к компании (Company Match)**:
   - Если роль пользователя **не глобальный читатель**, то для любой операции с терминалом проверяется совпадение компании пользователя и компании терминала: `user.companyId == terminal.companyId`. Если они не совпадают — возвращается `403 Forbidden`.
   - **Глобальные читатели — `SYSTEM_ADMIN` и `AUDITOR`** (`PaymentLinkService.isGlobalReader`, с 15.08.2026): Company Match для них отключается, доступ ко всем терминалам. Для `AUDITOR` это только чтение — на запись он отсекается раньше, проверкой списка разрешённых ролей.
...
5. **Получение списка ссылок (`GET /api/v1/payment-links`)**:
   ...
   - Если фильтр не передан, то для глобальных читателей (`SYSTEM_ADMIN`, `AUDITOR`) возвращаются все ссылки. Для остальных ролей выборка автоматически ограничивается терминалами компании пользователя:
     `SELECT pl FROM PaymentLink pl WHERE pl.terminalId IN (SELECT t.id FROM Terminal t WHERE t.companyId = :companyId)`
6. **Получение списка транзакций (`GET /api/v1/transactions?page=&size=`)**:
   - Роли: разрешено всем пяти (`READ_ROLES`); любая другая строка в `role` → `403 Forbidden`.
   - Для глобальных читателей возвращаются транзакции всех компаний (`TransactionRepository.findAllBy`).
   - Для остальных ролей выборка ограничивается терминалами компании: `TransactionRepository.findByLink_TerminalIdIn(allowedTerminals, pageable)`.
   - Если у пользователя нет `companyId` или у компании нет ни одного терминала — `200 OK` с пустой страницей, а не ошибка.
   - Оба запроса идут с `@EntityGraph(attributePaths = "link")`: маппер обращается к ленивому `link` на каждой строке, без графа получается N+1.
   - Серверных фильтров (терминал, статус, даты) у эндпоинта нет — фронтенд фильтрует на клиенте.
7. **DMS Clearing (`/complete`)**:
   - Роли: разрешено всем, кроме `AUDITOR`.
   - Валидация: выполняется Company Match.
8. **Возвраты (`/refund`)**:
...
- **Срок действия (`expiresAt`)**: Выставляется **всегда** (P1-9, 18.08.2026). Если мерчант не передал `expiresAt` при создании, ссылка живёт `pbl.link.default-ttl` (по умолчанию 24 часа). Свой срок можно задать и при создании, и через `PATCH`; он обязан быть в будущем и не дальше `pbl.link.max-ttl` (по умолчанию 90 дней) **от `created_at` ссылки** — в `update` потолок считается от даты создания, иначе цепочкой PATCH'ей срок продлевался бы бесконечно. Нарушение любой из границ — 400 с указанием предельной даты. Проверяется в момент перехода `/open`: если транзакция переведена в `PENDING` до `expiresAt`, оплата принимается, даже если редирект от эквайера произошел после наступления `expiresAt`. Просроченные `ACTIVE`-ссылки фоново переводит в `EXPIRED` `PaymentLinkScheduler` (каждые 5 минут). `NULL` в `expires_at` (ссылки, созданные до 18.08.2026) означает «без срока».
- **Реактивация ссылок**: 
...
- `amount` (Decimal) — Авторизованная сумма транзакции. После capture **не меняется**: это история операции.
- `captured_amount` (Decimal, nullable) — Сколько реально списано с карты при клиринге. Меньше `amount` при частичном capture; `NULL` у SMS-платежей (стадии capture нет) и до capture. От этого поля считается потолок возврата.
...
- Транзакция должна быть в статусе `AUTHORIZED`. `PENDING` не отказ сразу: сначала опрос эквайера, решение по свежему статусу. `SUCCESS` — отказ, повторный capture запрещён (P0-7).
- Валидация суммы (P0-8): не больше авторизованной, не больше двух знаков после запятой. Ноль и отрицательные отсекает `@Positive` на DTO.
- Сервис загружает учетные данные терминала (`login`/`password`) по `terminal_id` платежной ссылки и отправляет запрос списания провайдеру — **с суммой**.
- **Частичный capture поддерживается.** Захваченная сумма пишется в `captured_amount`, `amount` остаётся авторизованной. Статус — `SUCCESS`, отдельного статуса под частичный capture нет. Неиспользованный остаток холда провайдеру не отменяется (Void не реализован).

### 3.3. Возвраты (Refunds)
- Вызывается через: `POST /api/v1/transactions/{transactionId}/refund`.
- Валидация: статус `SUCCESS` или `PARTIALLY_REFUNDED`, сумма не больше двух знаков после запятой, а сумма возвратов не превышает **захваченную** сумму: `captured_amount`, а при `NULL` (SMS) — `amount`. Считать потолок от `amount` при частичном capture — значит вернуть деньги, которые с карты не списывались (P0-8).
- Сервис загружает учетные данные терминала и отправляет запрос возврата провайдеру.
- Обновление статусов: поле `refunded_amount` увеличивается, при полном возврате захваченной суммы статус -> `REFUNDED`, иначе -> `PARTIALLY_REFUNDED`.
...
- **Request Payload**:
  ...
  ```
- `amount` — строка с ровно двумя знаками (`setScale(2, UNNECESSARY).toPlainString()`), как и в Refund.
  `BigDecimal.toString()` здесь использовать нельзя: он даёт научную нотацию (`1E+3`).
  Поддержка `amount` в этой фазе подтверждена MilliKart 17.08.2026; до этого сумма не передавалась
  и эквайер всегда клирил полный холд.
