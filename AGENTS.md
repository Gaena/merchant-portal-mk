...
├── ecom/                   # :8083  — вкладка E-commerce: чтение схемы шлюза провайдера
...
только `ACTIVE`), `App.tsx`, `TransactionDetailPage`, `HomePage`, `FilterPanel` и
`PayByLinkDetailPage` (берут **все**, включая заблокированные, — иначе у старого платежа пропадёт
подпись терминала). Врезок со списками в `SettingsPage`, бравших
...
- **`GET /api/v1/terminals/options` отдаёт заблокированные терминалы, и это не недосмотр**
  (Р-45). Лёгкий список (`id`, `name`, `login`, `status`) кормит нескольких потребителей с разными
  нуждами: форме создания ссылки нужны только `ACTIVE` — она и фильтрует; экрану транзакций,
  карточке транзакции, главной и фильтру по терминалу нужны **все**, потому что они подписывают
  терминал в старых платежах, и по снятому с обслуживания терминалу подпись должна остаться.
  Отфильтруешь на сервере — на экране транзакций вместо подписей появятся `#N`.
...
**На чём они идут.** По умолчанию — H2 в режиме совместимости с PostgreSQL. Это эмуляция, и там,
где вопрос теста именно в поведении СУБД, она отвечает не за ту схему: у H2 нет частичных
индексов, свой тип под `jsonb`, своя семантика блокировок под `SELECT ... FOR UPDATE`, и
миграции до выката не исполняются ни разу на том, что стоит в проде. Поэтому часть тестов
переведена на **настоящую PostgreSQL в контейнере** (12.09.2026). Им нужен запущенный Docker.

| Тест | Модуль | Почему не H2 |
|:---|:---|:---|
| `TransactionIndexSchemaTest` | `pbl` | индексы, созданные миграцией, проверяются по метаданным той схемы, что будет в проде |
| `AuditLogSchemaTest` | `directory` | то же для индексов журнала |
| `SharedSchemaMigrationTest` | `directory` | идемпотентность миграций: «повторный `ADD COLUMN` падает» — свойство PostgreSQL, не H2 |
| `OpenLinkConcurrencyTest` | `pbl` | гонка на `SELECT ... FOR UPDATE`; у H2 своя семантика блокировок |
| `RefreshTokenConcurrencyTest` | `auth` | то же: второй коммит обязан увидеть работу первого |
| `MoneyOperationsIntegrationTest` | `pbl` | след возврата, метка списания и история операции лежат в `jsonb`; плюс точность `numeric` при частичном списании |
| `UserListPaginationTest` | `auth` | порядок страниц и поиск через LIKE: сортировка строк в PostgreSQL зависит от локали базы, H2 сравнивает побайтово |
| `DirectoryListPaginationTest` | `directory` | то же |
| `AuditLogQueryTest` | `directory` | то же плюс срезы по времени |
| `TerminalBlockingIntegrationTest` | `directory` | пишет в `payment_links` чужого модуля нативным запросом и проверяет, что блокировка двигает ссылки |
| `TerminalBlockedIntegrationTest` | `pbl` | обратная сторона той же блокировки, со стороны платёжного пути |
| `TransactionReconciliationIntegrationTest` | `pbl` | метки исхода свёртки лежат в `jsonb`, выборка идёт по возрасту операции |
| `DashboardSummaryTest` | `pbl` | группировки с границами суток в часовом поясе Баку: раскладка по окнам у двух СУБД разная |
| `PaymentLinkRefundUsageTest` | `pbl` | точность `numeric`: сколько возвращено, сколько осталось, когда возврат полный |
...
Что об этом надо знать, прежде чем добавлять пятый:

- Контейнер объявлен один раз в `common` (`testFixtures`, `PostgresTestContainer`) и статичен:
  один на JVM, версия образа прибита и должна совпадать с продовой. Контейнер на класс превратил
  бы минуту тестов в минуты.
- Он помечен `@TestConfiguration`, **а не `@Configuration`**, и это не косметика. Сервисы
  сканируют `az.millikart` целиком, куда попадает и он: обычная `@Configuration` подхватилась бы
  во всех тестах разом, весь набор молча уехал бы с H2 на контейнер и начал делить одну базу
  между классами. Так уже случилось при переезде — поймали по упавшему тесту блокировки
  терминала, где ссылки повели себя иначе.
- Подключается явным `@Import(PostgresTestContainer.class)` рядом с `@SpringBootTest`.
  Тестам без Spring (`SharedSchemaMigrationTest`) контейнер доступен через `instance()`, и каждый
  метод там работает в **своей схеме**: миграции должны видеть пустоту, как при первом
  развёртывании.
- Первое же, что вскрылось после переезда: имена таблиц. H2 складывает неэкранированные
  идентификаторы в верхнем регистре, PostgreSQL — в нижнем, и метаданные JDBC сравнивают их
  буквально. Тесты схемы искали `"TRANSACTIONS"` и `"AUDIT_LOGS"`.
- Версия Testcontainers задана в корневом `build.gradle` **выше** той, что приносит BOM Spring
  Boot 3.2.5, и тестовой задаче передаётся `api.version=1.40`. Причина: Testcontainers 1.19.x
  ходит в Docker по версии API 1.32, а Docker Engine 29 объявляет минимальной 1.40 и отвечает
  на такие запросы 400 — наружу это выглядит как «Could not find a valid Docker environment»
  при полностью рабочем докере. Обновится Boot — обе строки можно будет снять.
