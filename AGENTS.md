...
- **SQL `ecom` из портала на Oracle провайдера ещё не исполнялся** (Р-74…Р-79). Выписка проверена
  тестами на выгрузке стенда (114 заказов во всех статусах, с возвратами и реверсалами), справочник
  терминалов — только тестами на форму запроса. Chargeback не разбирается.
