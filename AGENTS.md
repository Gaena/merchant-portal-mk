...
- **Одна строка выписки на заказ, а не на операцию.** Без группировки по `orderid` DMS-платёж
  задваивается, суммы тоже, а пагинация едет; токен свёрнут подзапросом по той же причине.
- **Скоуп — только `EcomScopeService`:** компания → её терминалы → `merchant_rid`. Пустой список —
  пустая выписка; ветки «терминалов нет, значит показать всё» быть не должно.
- **`o.password` из схемы шлюза не попадает никуда** — ни в выписку, ни в экспорт, ни в логи.
  Период обязателен и ограничен, страница — курсорная (время последней операции + номер заказа).
...
- **`entityType` и `action` — только константы `AuditEntity` / `AuditAction`** (P3-2). Новое
  ...
  Соглашения: `entityId` у `AUTH` — всегда логин; `"ALL"` — действие над списком; смена статуса —
