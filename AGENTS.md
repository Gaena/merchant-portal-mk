...
- **SQL `ecom` из портала на Oracle провайдера ещё не исполнялся** (Р-74…Р-78). Выписка проверена
  тестами на выгрузке стенда (114 заказов во всех статусах, с возвратами и реверсалами). Справочник
  терминалов провайдера — на предварительном запросе. Chargeback не разбирается.
